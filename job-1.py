...
class MixedSpectrumSimulator:
    def __init__(self, simulators: List[SpectrumSimulator], config: Dict, delta_E_max: Dict[str, float], element_map_labels: Dict[str, List[float]]):
        ...
        self.wavelengths = np.linspace(self.wl_range[0], self.wl_range[1], self.resolution, dtype=np.float32)
        self.device = torch.device("cpu")
        self.intensity_threshold = 0.008  # Menggunakan threshold yang lebih rendah
    ...
    def generate_sample(self, ionization_energies: Dict[str, float], selected_base_elements: List[str]) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]]:
        ...
        # Cari spektrum untuk suhu yang relevan dari setiap simulator
        for sim_idx, simulator in enumerate(selected_simulators):
            try:
                temp_idx = simulator.temperatures.index(temp)
                ...
                _, all_spectra, _, _, _, _, all_contributions = simulator.simulate(atom_percentage)
                
                if temp_idx < len(all_spectra):
                    mixed_spectrum += all_spectra[temp_idx]
                    element_contributions[sim_idx] = all_contributions[temp_idx]
            except ValueError:
                self.logger.warning(f"Suhu {temp}K tidak ditemukan di simulator untuk {simulator.element_label}. Melewati.")
