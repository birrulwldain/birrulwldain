...
class SpectrumSimulator:
    ...
    def __init__(
        ...
        self.element_map_labels = element_map_labels or {}
        self.logger = logging.getLogger('SpectralSimulation')
        if ipex:
...
class MixedSpectrumSimulator:
    def __init__(self, simulators: List[SpectrumSimulator], config: Dict, delta_E_max: Dict[str, float], element_map_labels: Dict[str, List[float]]):
        ...
        self.wavelengths = np.linspace(self.wl_range[0], self.wl_range[1], self.resolution, dtype=np.float32)
        self.device = torch.device("cpu")
        self.intensity_threshold = 0.008 # >>> PERUBAHAN: Menurunkan threshold untuk mengurangi background
    ...
    # >>> PERUBAHAN: Fungsi generate_sample sekarang menerima bobot pemilihan (sampling_weights)
    def generate_sample(self, ionization_energies: Dict[str, float], sampling_weights: List[float]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Optional[Dict]]:
        ...
        # >>> PERUBAHAN: Meningkatkan jumlah elemen per spektrum untuk mengurangi background
        num_target_elements = np.random.randint(10, 15)
        self.logger.debug(f"Memilih {num_target_elements} elemen untuk sampel")
        
        # >>> PERUBAHAN: Menggunakan pemilihan berbobot (weighted sampling)
        selected_base_elements = np.random.choice(
            BASE_ELEMENTS, 
            num_target_elements, 
            replace=False, 
            p=sampling_weights
        ...
        self.logger.debug(f"Selesai memilih elemen target (berbobot): {selected_base_elements.tolist()}")
        ...
        for base_elem, elem_neutral, elem_ion in selected_pairs:
            ...
            # >>> CATATAN: Logika bobot intensitas yang salah sebelumnya sudah dihapus. Persentase tidak lagi diubah.
        ...
        atom_percentages = np.array([atom_percentages_dict[e] for e in selected_target_elements], dtype=np.float32)
...
class DatasetGenerator:
    ...
    def _hash_combination(self, temp: float, n_e: float, atom_percentages: Dict) -> str:
        ...
        combination_str = f"{temp:.5f}_{n_e:.5f}_{str(elements_sorted)}"
    ...
    def _generate_sample_params(self, delta_E_max_dict: Dict[str, float]) -> List[Tuple[float, float]]:
        ...
        for i, T in enumerate(self.temperature_range):
            ...
            max_ne_per_temp = min(len(valid_n_e), 10)
    ...
    # >>> PERUBAHAN: Task sekarang menerima bobot pemilihan (sampling_weights)
    def _generate_sample_task(self, args: Tuple[float, float, List[SpectrumSimulator], Dict[str, float], Dict[str, float], int, set, List[float]]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Optional[Dict], Optional[Dict]]:
        T, n_e, simulators, ionization_energies, delta_E_max, sample_id, used_combinations, sampling_weights = args
        logger = logging.getLogger('SpectralSimulation')
        logger.debug(f"Memulai pemrosesan sampel {sample_id} dengan T={T} K, n_e={n_e:.1e} cm^-3")
        mixed_simulator = MixedSpectrumSimulator(simulators, self.config, delta_E_max, self.element_map_labels)
        ...
        while attempt < max_attempts and not sample_generated:
            # >>> PERUBAHAN: Meneruskan bobot ke generate_sample
            result = mixed_simulator.generate_sample(ionization_energies, sampling_weights)
            if result[0] is None:
                attempt += 1
        ...
        if not sample_generated:
            ...
            T = np.random.choice(self.temperature_range)
            ...
            # >>> PERUBAHAN: Meneruskan bobot pada pemanggilan rekursif
            return self._generate_sample_task((T, n_e, simulators, ionization_energies, delta_E_max, sample_id, used_combinations, sampling_weights))
        else:
            logger.debug(f"Sampel {sample_id} selesai diproses")
        return result

    # >>> PERUBAHAN: Fungsi generate_dataset sekarang menerima bobot pemilihan
    def generate_dataset(
        ...
        np.random.seed(42)
        self.combinations_json_path = os.path.join(processed_dir, "combinations-35s.json")
        ...
        max_workers = 8 # Ditetapkan secara eksplisit
        self.logger.info(f"Menggunakan {max_workers} proses untuk pembuatan dataset")

        combinations_to_save = []
        with Pool(processes=max_workers) as pool:
            # >>> PERUBAHAN: Menambahkan sampling_weights ke argumen task
            tasks = [(T, n_e, simulators, ionization_energies, delta_E_max, i + 1, self.used_combinations, sampling_weights)
            ...
            for result in tqdm(pool.imap_unordered(self._generate_sample_task, tasks), total=len(tasks), desc="Menghasilkan dataset"):
                ...
                if spectrum is not None:
                    ...
                    if len(spectra_list) % (self.num_samples // 5) == 0 and len(spectra_list) > 0:
        ...
        # >>> PERUBAHAN: Logging distribusi elemen berdasarkan kemunculan di sampel
        element_appearance_counts = Counter()
        for ap in atom_percentages_list:
            # Mengambil elemen dari dictionary persentase atom untuk sampel ini
            elements_in_sample = [k.split('_')[0] for k in ap.keys() if k not in ['temperature', 'electron_density', 'delta_E_max']]
            element_appearance_counts.update(list(set(elements_in_sample))) # Gunakan set untuk menghitung per sampel
        self.logger.info("Distribusi kemunculan elemen di seluruh sampel:")
        for elem, count in element_appearance_counts.most_common():
            self.logger.info(f"{elem}: muncul di {count} sampel ({count/len(atom_percentages_list)*100:.2f}%)")
        ...
        # --- Bagian penyimpanan HDF5 (tidak ada perubahan logika) ---
        ...
        train_data = (spectra_array[train_idx], labels_array[train_idx], [atom_percentages_array[i] for i in train_idx])
        val_data = (spectra_array[val_idx], labels_array[val_idx], [atom_percentages_array[i] for i in val_idx])
        test_data = (spectra_array[test_idx], labels_array[test_idx], [atom_percentages_array[i] for i in test_idx])
        output_filename = "spectral_dataset-35s.h5"
        ...
        if os.path.exists(output_path):
            with h5py.File(output_path, 'r') as f:
                ...
                for ds_name in ['train', 'validation', 'test']:
                    if ds_name in f:
                        if 'spectra' in f[ds_name]: existing_data[ds_name]['spectra'] = f[ds_name]['spectra'][:]
                        if 'labels' in f[ds_name]: existing_data[ds_name]['labels'] = f[ds_name]['labels'][:]
                        if 'atom_percentages' in f[ds_name]: existing_data[ds_name]['atom_percentages'] = f[ds_name]['atom_percentages'][:]
        ...
        total_samples = total_existing_samples + len(spectra_list)
        with h5py.File(output_path, 'a') as f:
            for ds_name in ['train', 'validation', 'test']:
                if ds_name in f: del f[ds_name]
            if 'wavelengths' in f: del f['wavelengths']
...
def main():
    ...
    logger.info("Memulai simulasi spektral dengan logika penyeimbangan sampel")
    if ipex:
    ...
    # >>> MEKANISME BARU: Menghitung bobot pemilihan berdasarkan kerapatan garis <<<
    logger.info("Menghitung bobot pemilihan (sampling weights) untuk penyeimbangan elemen...")
    
    # 1. Hitung jumlah garis per elemen dasar
    line_counts = {elem: 0 for elem in BASE_ELEMENTS}
    for key, data in nist_data_dict.items():
        base_elem = key.split('_')[0]
        if base_elem in line_counts:
            line_counts[base_elem] += len(data)

    logger.info("Jumlah garis spektral per elemen dasar:")
    for elem, count in sorted(line_counts.items(), key=lambda item: item[1]):
        logger.info(f"  - {elem}: {count} garis")

    # 2. Hitung bobot terbalik (inverse weight)
    sampling_weights = {}
    for elem in BASE_ELEMENTS:
        count = line_counts.get(elem, 0)
        # Menggunakan log10 untuk meredam perbedaan dan +1 untuk menghindari log(0)
        # Bobot tinggi untuk elemen dengan garis sedikit
        sampling_weights[elem] = 1.0 / np.log10(1 + count) if count > 0 else 1.0

    # 3. Normalisasi bobot menjadi distribusi probabilitas (total = 1)
    total_weight = sum(sampling_weights.values())
    normalized_weights = [sampling_weights[elem] / total_weight for elem in BASE_ELEMENTS]
    
    logger.info("Probabilitas pemilihan akhir (setelah normalisasi):")
    for i, elem in enumerate(BASE_ELEMENTS):
        logger.info(f"  - {elem}: {normalized_weights[i]:.4f}")
    # >>> AKHIR MEKANISME BARU <<<
    ...
    # >>> PERUBAHAN: Meneruskan bobot yang sudah dinormalisasi ke generator
    generator.generate_dataset(
        ...
        element_map_labels=element_map,
        sampling_weights=normalized_weights # Meneruskan bobot ke fungsi
    )
