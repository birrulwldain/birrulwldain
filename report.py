...
# import scipy.signal # Import for potential peak finding
...
#             # --- MODIFIKASI DIMULAI: Anotasi Lebih Baik ---
#             annotations_data = [] # Untuk menyimpan data anotasi sebelum penempatan
...
#                     peaks, properties = scipy.signal.find_peaks(element_spectrum_values, height=0.05 * np.max(spectrum), prominence=0.01 * np.max(spectrum), distance=5)
...
#             # Anotasi titik-titik yang telah dikumpulkan, dengan penyesuaian posisi dasar untuk mengurangi tumpang tindih
#             annotations_data.sort(key=lambda x: x['wavelength']) # Urutkan berdasarkan panjang gelombang

#             # Struktur untuk melacak posisi Y yang telah digunakan di area tertentu
...
#                 element_name = ann['element']
#                 wavelength = ann['wavelength']
#                 intensity = ann['intensity']
#                 color = ann['color']

#                 base_y_offset = 15 # Offset dasar dari titik data
...
#                 wavelength_bin = int(wavelength / 5) * 5 # Contoh: bin setiap 5 nm

#                 current_y_offset = base_y_offset
#                 # Cek apakah ada anotasi lain yang tumpang tindih secara horizontal
#                 for wl_bin_check in range(wavelength_bin - 5, wavelength_bin + 10, 5): # Cek bin sekitar
#                     if wl_bin_check in used_y_positions:
#                         if current_y_offset in used_y_positions[wl_bin_check]:
#                             current_y_offset += 15 # Pindah ke atas jika slot ini sudah dipakai
